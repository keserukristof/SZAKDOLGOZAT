...
var allDayCellsData = function (viewCellsData, groups, groupOrientation) { return groupOrientation !== VERTICAL_GROUP_ORIENTATION
    || !groups
    ? allDayCells(viewCellsData)
    : allDayCellsFromViewCellsAndGroups(viewCellsData, groups); };
var allDayCellsFromViewCellsAndGroups = function (viewCellsData, groups) { return groups[groups.length - 1].reduce(function (acc, group, index) {
...
var expandCellsWithGroupedByDateData = function (viewCellsData, groups, sortedResources) { return viewCellsData.map(function (cellsRow) { return cellsRow.reduce(function (acc, viewCell) {
    var groupedCells = groups[groups.length - 1].map(function (group, index) { return addGroupInfoToCell(group, groups, sortedResources, viewCell, index, false, HORIZONTAL_GROUP_ORIENTATION); });
...
var expandHorizontallyGroupedCells = function (viewCellsData, groups, sortedResources) { return groups[groups.length - 1].reduce(function (acc, group, index) {
    if (index === 0) {
...
var expandVerticallyGroupedCells = function (viewCellsData, groups, sortedResources) { return groups[groups.length - 1].reduce(function (acc, group, index) {
    if (index === 0) {
...
var getVerticalRowFromGroups = function (groups, groupIndex, groupingPanelRowSpan, timeTableCellHeight) { return groups.reduce(function (acc, groupColumn, columnIndex) {
...
var getLabelsForAllGroups = function (cellsData, groups, groupOrientation) {
    ...
    return groups[groups.length - 1].reduce(function (acc, group, groupIndex) { return __spread(acc, [
...
var cellData = function (timeTableIndex, allDayIndex, viewCellsData, groups, groupOrientation) {
    if (allDayIndex !== -1) {
...
var scroll = function (offset, firstBoundary, secondBoundary, changeScroll) {
