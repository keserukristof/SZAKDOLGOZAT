...
var useListField = function useListField(state, dispatch) {
  ...
  var setValue = function setValue() {// dispatch(actions.updateValue(fieldArgs.name, v))
  ...
  var fieldData = state.getIn(['items'], (0, _immutable.List)()).map(function (item) {
    return (0, _resolveFieldData.resolveFieldData)(item, dispatch, getAllValues);
  }).toArray();
  return {
    props: {
      ...
      items: fieldData.map(function (item, index) {
        return (0, _objectSpread2.default)({}, (0, _resolveFieldData.getFieldProps)(item, state.getIn(['items', index])), {
          key: "".concat(fieldArgs.name, ".").concat(index)
        });
      }),
      add: add,
      remove: remove,
