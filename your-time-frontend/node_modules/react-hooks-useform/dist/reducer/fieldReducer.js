...
var fieldReducer = function fieldReducer(state, _ref) {
  ...
  var handlers = (_handlers = {}, (0, _defineProperty2.default)(_handlers, actionTypes.updateValue, function (value) {
    setTimeout(function () {
      ...
      state.getIn([listenersKey, fieldName], (0, _immutable.List)()).forEach(function (listener) {
        return listener(value, previous, state.get('formTools').current);
      });
    });
    return state.setIn([fieldsKey].concat((0, _toConsumableArray2.default)(fieldPath), [current, 'value']), value).setIn([fieldsKey].concat((0, _toConsumableArray2.default)(fieldPath), [current, 'pristine']), value == state.getIn([fieldsKey, fieldName, 'initial', 'value']));
...
var getFieldPath = function getFieldPath(fieldName) {
  ...
  if (match) {
    ...
    var rest = fieldName.substr(match.index + 1);
    fieldPath = [firstField].concat((0, _toConsumableArray2.default)(rest.split('.')));
  }
